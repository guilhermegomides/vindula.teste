...
class CriandoListaDeRestaurantes(unittest.TestCase):
    def setUp(self):
...
#        self.accept_next_alert = true
    ...
    def test_criando_lista_de_restaurantes(self):
        driver = self.driver
	...
	print '-'*80
	print 'Adicionando Aplicacao'
	print '-'*80
        driver.find_element_by_css_selector("a[title=\"Adiciona novos itens dentro deste item\"] > span").click()
        driver.get(self.base_url + "/vindula/pasta-de-testes/++add++vindula.food.restaurantes")
        driver.find_element_by_id("form-widgets-IBasic-title").clear()
        driver.find_element_by_id("form-widgets-IBasic-title").send_keys("Teste Restaurante")
